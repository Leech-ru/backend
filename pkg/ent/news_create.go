...
func (nc *NewsCreate) SetImageID(u uuid.UUID) *NewsCreate {
	nc.mutation.SetImageID(u)
	return nc
}
...
func (nc *NewsCreate) check() error {
	if _, ok := nc.mutation.ImageID(); !ok {
		return &ValidationError{Name: "image_id", err: errors.New(`ent: missing required field "News.image_id"`)}
	}
...
func (nc *NewsCreate) createSpec() (*News, *sqlgraph.CreateSpec) {
	...
	if value, ok := nc.mutation.ImageID(); ok {
