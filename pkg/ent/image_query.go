...
func (iq *ImageQuery) QueryCosmetics() *CosmeticsQuery {
	query := (&CosmeticsClient{config: iq.config}).Query()
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withCosmetics: iq.withCosmetics.Clone(),
...
func (iq *ImageQuery) WithCosmetics(opts ...func(*CosmeticsQuery)) *ImageQuery {
	query := (&CosmeticsClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withCosmetics = query
	return iq
}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withCosmetics; query != nil {
		if err := iq.loadCosmetics(ctx, query, nodes,
			func(n *Image) { n.Edges.Cosmetics = []*Cosmetics{} },
			func(n *Image, e *Cosmetics) { n.Edges.Cosmetics = append(n.Edges.Cosmetics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadCosmetics(ctx context.Context, query *CosmeticsQuery, nodes []*Image, init func(*Image), assign func(*Image, *Cosmetics)) error {
	...
	query.Where(predicate.Cosmetics(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.CosmeticsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.image_cosmetics
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_cosmetics" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_cosmetics" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
